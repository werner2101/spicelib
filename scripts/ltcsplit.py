...
def read_lib(lib_fn, basedir):
  ...
  while True:
    ...
    try:
      ...
      name = re.search('^\.SUBCKT\\s+(.*?)\\s+', line, re.IGNORECASE).groups()[0]
      ...
      while True:
        ...
        STANDARD_SUBSTITUTIONS = [
          #LTC spice files use ; to denote inline comments.  Gnucap uses "
          #change them with this substitution.  Also, include leading space
          #XXX: gnucap seems to use " in some places and ' in others.  For now
          #delete all inline comments
          ('(^[^*][^;]*);(.*$)', '\\1'),
          #LTSpice uses TC for the first order time coefficient; gnucap uses TC1
          ('(^R.*\\s)TC(=.*$)', '\\1TC1\\2'),
          #Gnucap does not allow - in subckt names.  Replace it with _
          (name, new_name),
          #LTSpice assumes PMOS and NMOS models are level 1 by default, gnucap
          #requires an explicit level assignment
          ('(^\.MODEL .*[NP]MOS\()(?!.*LEVEL)', '\\1LEVEL=1 '),
          #Gnucap Assumes that Q x y z w v to mean Q C B E S model, but LTSpice
          #means Q C B E model area.  To fix this, we explicitly label the
          #area
