...
download_ltc = env.Command('downloads/ltc/LTC.zip', None, 'wget -P `dirname $TARGET$` http://ltspice.linear.com/software/LTC.zip')

unpack_ltc = env.Command(None, download_ltc,
[
    Delete(os.path.join(TEMPDIR, 'ltc')),
    Mkdir(os.path.join(TEMPDIR, 'ltc')),
    """
    md5sum %(source)s > %(MODEL_SIGDIR)s/ltc_all.md5sum
    unzip -d %(TEMPDIR)s/ltc %(source)s
    scripts/ltcsplit.py -d %(TEMPDIR)s/ltc %(TEMPDIR)s/ltc/LTC.lib
    """ % {'TEMPDIR': TEMPDIR, 'MODEL_SIGDIR': MODEL_SIGDIR, 'source': '$SOURCE'}
])

env.Alias('unpack_ltc', unpack_ltc)

#if LTC is already unpacked, generate new targets
...
create_ltc_opamps = []
for fn in ALL_LTC_OPAMPS:
    source = os.path.join('unpack', 'ltc', fn)
    if os.path.isfile(source):
        target = os.path.join(MODEL_LIBDIR, 'ltc', 'opamps', fn)
        create_ltc_opamps.append(env.Command(target, source,
            """
            cp $SOURCE $TARGET
            md5sum $TARGET >> %(MODEL_SIGDIR)s/ltc_opamps_lib.md5sum
            """ % {'MODEL_SIGDIR': MODEL_SIGDIR}))

env.Alias('create_ltc_opamps', create_ltc_opamps)
